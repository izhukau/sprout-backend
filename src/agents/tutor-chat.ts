...
import { db } from "../db";
import { assessments, questions, answers, userNodeProgress, nodes, nodeEdges } from "../db/schema";
  ...
  const tools: AgentTool[] = [
    ...
    },
    {
      ...
      input_schema: {
        ...
        properties: {
          what: {
            type: "string",
            description: "What to visualize (e.g., 'matrix multiplication process', 'tree traversal order')",
          },
          style: {
            type: "string",
    ...
    },
    {
      ...
      async execute() {
        ...
        if (!diagnosticResults.length) {
          return JSON.stringify({ diagnosticScore: null, message: "No diagnostic found for this concept." });
        }
        ...
        const answerByQuestion = new Map(studentAnswers.map((a) => [a.questionId, a]));
        ...
        const incorrectQuestions = scoredItems
          .filter((i) => i.isCorrect === false || (i.score !== null && i.score < 0.7))
    ...
    },
    {
      ...
      async execute() {
        ...
        if (!incomingEdges.length) {
          return JSON.stringify({ prerequisites: [], allMastered: true, message: "No prerequisites for this subconcept." });
        }
        ...
        const progressByNode = new Map(prereqProgress.map((p) => [p.nodeId, p]));
    ...
    },
    {
      ...
      input_schema: {
        type: "object" as const,
        properties: {
          exercise_prompt: { type: "string", description: "The exercise question that was asked" },
          ...
          feedback: { type: "string", description: "Feedback given to the student" },
        },
      ...
      async execute(input: {
        ...
        const averageScore = allScores.length
          ? Number((allScores.reduce((a, b) => a + b, 0) / allScores.length).toFixed(3))
          : input.score;
